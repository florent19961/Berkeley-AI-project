...
class PerceptronClassifier(object):
    ...
    def set_weights(self, weights):
        if not isinstance(weights, dict): ### j'ai rajouté le not je pense que c'etait une erreur puisqu'on veut que ce soit un dict
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                if callback is not None: 
                    ...
                    # Quel est l'objectif du perceptron ? Il est censé prendre en entrée des occurences de input_train_data puis
                    # il calcule la somme des composantes de ces occurences multipliée par le vecteur des poids de chaque classes
                    # on obtient ainsi en sortie les valeurs données par le perceptron pour chaque classe
                    # on en déduit ensuite par comparaison avec le label dans label_train_data comment actualiser le poids

                    # ainsi notre self.weights c'est un dictionnaire contenant 10 paires clés valeurs représentant chacune une classe
                    # chacune de ces paires possède en valeurs un util.Counter lequel est lui meme composé de self.num_features paires
                    # clés valeurs, chacune représentant le numéro du poids et le poids associé
                    # On doit alors effectuer la passe avant consistant a multiplier les vecteurs weights de chaque feature avec l'input
                    outputs = []
                    for class_weights in self.weights.values():
                        #print(class_weights)
                        #print(type(class_weights), type(self.weights))
                        outputs.append(class_weights * input_train_data[i])
                    # on a maintenant une liste contenant tous les outputs.

                    output = max(outputs)
                    #print(outputs)
                    index_output = outputs.index(max(outputs))
                    if index_output != label_train_data[i]:
                        self.weights[index_output] += input_train_data[i]
                        self.weights[index_output] -= input_train_data[i]
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        dic_weights = self.weights[label]
        weights = []
        for value in dic_weights.values():
            weights.append(value)
        #print((weights))
        
        for i in range(100):
            best100Features.append(weights.index(max(weights)))
